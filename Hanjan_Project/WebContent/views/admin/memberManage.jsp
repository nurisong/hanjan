    ...
    <div class="main-outer">
        <div>
            ...
            <table class="list-area" align="center">
                <tr>
                    ...
                    <a class="btn btn-sm btn-info" href="#">선택 회원 탈퇴</a>          
                    <br>
                        </div>
                            <br>
                    </td>
				...
				<% }  else { %>
					...
					<% for(Member m : list) {%>
		       			   <tr>
         						...
         						<td><%= m.getSaleSum() %> </td>
         						<td><%= m.getPoint() %> </td>
								<td><input type="checkbox" name="check[]" class="chk"></td>
						</tr>
					<% } %>
				<% } %>
     ...
     <script>
        $(function(){
			...
			<% if(currentPage != 1){ %>
				<button onclick="location.href='<%=contextPath %>/MemberManage.do?cpage=<%= currentPage - 1 %>'">&lt;</button>
			<% } %>	
			<% for(int i = startPage; i <= endPage; i++){ %>
				<% if(currentPage != i ){ %>
					<button onclick="location.href='<%= contextPath %>/MemberManage.do?cpage=<%=i %>'"><%=i %></button>
				<% } else{ %>
