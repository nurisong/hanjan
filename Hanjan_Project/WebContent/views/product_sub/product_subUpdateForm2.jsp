<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="com.hj.product_sub.model.vo.Subscription, java.util.ArrayList, com.hj.product_sub.model.vo.SubscriptionDetail" %>    
<%
	Subscription subscription=(Subscription)request.getAttribute("Subscription");
	ArrayList<SubscriptionDetail> pList=(ArrayList)request.getAttribute("pList");
	...
	for(int i=0;i<pList.size();i++){
		productList[i]=pList.get(i).getProductNo();
	}
	if(pList.size()!=6){
		for(int i=pList.size();i<6;i++){
			productList[i]=0;
		}
	}
	...
	<div class="main-outer">
        ...
        <div>
			...
			<div class="outer">	
		        ...
		        <form action="<%=contextPath%>/enrollUpdateLast.sub" method="post" >
		            ...
		            <div class="enrollouter">
		                ...
		                <table id="enrolltable">
		                    ...
		                    <tr>
		                        ...
		                        <td class="textForm"><input type="text" class="textForm2 addProductName" id="firstPro" required name="proPrice" value=<%=pList.get(0).getProductName() %> readonly ></td>
		                        <td>
		                   ...
		                   <% } else{ %>
		                   	<tr>
		                        ...
		                        <td><input class="deleteButton" type="button" value="줄삭제" onclick="return validate()" ></td>
		                    </tr>
		                   <% } %>
		                </table>
						...
						<div class="addTable">
							...
							<%if(!pList.isEmpty() ) {%>
								...
								<%for(int i=1;i<pList.size();i++) {%>
									<tr>
										<th class="addrow fontcontroll">넣을상품</th>
										<td class="addtextForm"><input type="text" class="addtextForm2 addProductName" name="proPrice" value="<%=pList.get(i).getProductName() %>" readonly></td>									
									</tr>
								<%} %>
							<% } %>	
							<%if(pList.size()!=6 && pList.size()!=0){ %>
								<tr>
									<th class="addrow fontcontroll">넣을상품</th>
									<td class="addtextForm"><input type="text" class="addtextForm2 addProductName" name="proPrice" readonly></td>									
								</tr>
							<%} %>
							</table>														
						</div>
						...
						<!-- 다음으로 넘겨줄 히든박스 -->
						<input type="hidden" name="subscriptNo" value="<%=subscription.getSubscriptNo()%>" >
						<input type="hidden" name="addProductNo1" value="<%=productList[0] %>"   >
						<input type="hidden" name="addProductNo2" value="<%=productList[1] %>">
						<input type="hidden" name="addProductNo3" value="<%=productList[2] %>">
						<input type="hidden" name="addProductNo4" value="<%=productList[3] %>">						
						<input type="hidden" name="addProductNo5" value="<%=productList[4] %>">
						<input type="hidden" name="addProductNo6" value="<%=productList[5] %>">
						<%if(pList.isEmpty()) {%>
							<input type="hidden" name="isNull" value="1">
						<%} else{%>
							<input type="hidden" name="isNull" value="2">
						<%} %>
		                <div align="center" class="completeScope">
		                    <input type="submit" value="수정" class="completeButton" onclick="return validate()">
		                </div>
		            </div>
	  ...
	  $(function(){
	  		...
	  		$(document).on('click','.selectProduct',function(){
	  			...
	  			if($('input[class~="addProductName"]').length <=5){
		  			$('.addTable2').append('<tr><th class="addrow fontcontroll">넣을상품</th><td class="addtextForm"><input type="text" class="addtextForm2 addProductName" name="proPrice"  readonly></td></tr>')
	  			}
	  			$('#closeModal').click();
				$('#popupTable').html('');
	  			$('.popupSearch').val('');
	  		...
	  		$('.deleteButton').on('click',function(){
	  			...
	  			$('.addTable2').children().children().eq($('input[class~="addProductName"]').length-2).remove();
				$('input[name=addProductNo'+($('input[class~="addProductName"]').length+1)+']').val('0');	
