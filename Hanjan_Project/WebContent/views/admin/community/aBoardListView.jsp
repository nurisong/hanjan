...
<body>
    ...
    <div class="main-outer" align="center">
        ...
        <div class="outer">
			...
			<table class="list-area type02" align="center" border="1" style="width:840px; margin-top:0px;"> 
				<thead>
					<tr>
			...
			<script>
				...
				$(function(){
					$('.list-area>tbody>tr').click(function(){
						...
						location.href='<%=contextPath%>/detail.bo?bno='+ $(this).children().eq(1).text();
					});
				});
				...
				function deleteBoard(){
					...
					$.ajax({
						...
						success : function(result){
							if(result > 0){
								location.href = "<%=contextPath%>/aList.bo?cpage=1"
							} else{
								alert("삭제 실패");
							}
						}
				...
				$(function(){
					...
					$('#comm1').css('font-weight', '600');
					$('#comm1').css('font-size', '21px');
					$('#comm1').css('color', 'grey');
				})
			...
			<%if(!list.isEmpty()){%>
				<div align="center" class="paging-area">
					<% if(currentPage != 1){ %>
						<button onclick="location.href='<%=contextPath %>aList.bo?cpage=<%= currentPage - 1 %>'">&lt;</button>
					<% } %>	
					<% for(int i = startPage; i <= endPage; i++){ %>
						<% if(currentPage != i ){ %>
							<button onclick="location.href='<%= contextPath %>/aList.bo?cpage=<%=i %>'"><%=i %></button>
						<% } else{ %>
